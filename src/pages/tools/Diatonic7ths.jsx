...
import { seventhMarkers } from '../../components/Fretboard/markers.js';
...
export default function Diatonic7ths({ onClose }) {
  const { currentKey, track, settings } = useAppState();
  const [highlighted, setHighlighted] = useState([]);
  const [showNotes, setShowNotes] = useState(false);
  const [range, setRange] = useState([0, 24]);

  const items = getSeventhsFor(track, currentKey);
  const markers = seventhMarkers(items, { highlighted, showNotes });
